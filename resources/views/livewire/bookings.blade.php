<div>
    <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
        <x-data-table :items='$items' :procedures='$procedures' :title="'por tarjeta'" :sortBy='$sortBy' :sortAsc='$sortAsc' :data="['card'=>'', 'procedure'=>'title']" :actions="['new', 'edit', 'delete']" />
    </div>
    ...
    <x-dialog-modal wire:model="confirmingItemAdd">
        ...
        <x-slot name="content">
            <div class="">
                ...
                <x-input id="card" type="text" wire:model="item.card" class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                <x-input-error for="item.card" class="mt-2" />
            </div>
            <div class="mt-4">
                <x-label for="procedure" value="{{ __('Ticketing') }}" />
                <select wire:model.defer="item.procedure_id" id="procedure" class="mt-2 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded shadow-sm mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value="0">
                        {{ __('- Seleccionar -') }}
                    </option>
                ...
                <x-input-error for="item.procedure_id" class="mt-2" />
            </div>
        </x-slot>
